        ...
        + 'static,
{
    ...
    Router::new()
        .route(
            "/api/similar/:user_id",
            get(get_users_with_similar_words::<I, A>),
        )
        .route(
            "/api/similar/:user_id/recommend",
            get(get_recommended_words_for_user::<I, A>),
        )
        .route(
            "/api/stamps",
            get(stamps_get::<I, A>).post(stamps_post::<I, A>),
        )
        .route(
            "/api/trend/day/:day",
            get(get_trending_words_for_day::<I, A>),
        )
        .route(
            "/api/trend/day/today",
            get(get_today_trending_words::<I, A>),
        )
        .route(
            "/api/trend/month/:month",
            get(get_trending_words_for_month::<I, A>),
        )
        .route(
            "/api/trend/year/:year",
            get(get_trending_words_for_year::<I, A>),
        )
        .route(
            "/api/words",
...
) -> std::result::Result<(models::GetRecommendedWordsForUserPathParams,), ValidationErrors> {
...
#[tracing::instrument(skip_all)]
async fn get_recommended_words_for_user<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    ...
    A: apis::similar::Similar,
{
    ...
    let validation =
        tokio::task::spawn_blocking(move || get_recommended_words_for_user_validation(path_params))
            .await
            .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_recommended_words_for_user(method, host, cookies, path_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::similar::GetRecommendedWordsForUserResponse::Status200_OK(body) => {
...
) -> std::result::Result<(models::GetUsersWithSimilarWordsPathParams,), ValidationErrors> {
    path_params.validate()?;
...
#[tracing::instrument(skip_all)]
async fn get_users_with_similar_words<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    ...
    A: apis::similar::Similar,
{
    #[allow(clippy::redundant_closure)]
    let validation =
        tokio::task::spawn_blocking(move || get_users_with_similar_words_validation(path_params))
            .await
            .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_users_with_similar_words(method, host, cookies, path_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::similar::GetUsersWithSimilarWordsResponse::Status200_OK(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(models::StampsGetQueryParams,), ValidationErrors> {
...
where
    I: AsRef<A> + Send + Sync,
    A: apis::stamps::Stamps,
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || stamps_get_validation(query_params))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .stamps_get(method, host, cookies, query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::stamps::StampsGetResponse::Status200_SuccessfulRetrieval(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(models::StampsPostHeaderParams, models::NewStamp), ValidationErrors> {
    ...
    A: apis::stamps::Stamps,
{
    // Header parameters
    let header_params = {
        let header_x_forwarded_user = headers.get(HeaderName::from_static("x_forwarded_user"));
        ...
        let header_x_forwarded_user = match header_x_forwarded_user {
                ...
                Err(err) => {
                    return Response::builder()
                        ...
                        .body(Body::from(format!(
                            "Invalid header X-Forwarded-User - {}",
                            err
                        )))
                        .map_err(|e| {
                            error!(error = ?e);
            ...
            None => {
                return Response::builder()
                    .status(StatusCode::BAD_REQUEST)
                    .body(Body::from("Missing required header X-Forwarded-User"))
                    .map_err(|e| {
                        error!(error = ?e);
                        StatusCode::INTERNAL_SERVER_ERROR
                    });
    ...
    #[allow(clippy::redundant_closure)]
    let validation =
        tokio::task::spawn_blocking(move || stamps_post_validation(header_params, body))
            .await
            .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .stamps_post(method, host, cookies, header_params, body)
        .await;
...
) -> std::result::Result<(models::GetTodayTrendingWordsQueryParams,), ValidationErrors> {
    query_params.validate()?;
...
#[tracing::instrument(skip_all)]
async fn get_today_trending_words<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    ...
    A: apis::trend::Trend,
{
    #[allow(clippy::redundant_closure)]
    let validation =
        tokio::task::spawn_blocking(move || get_today_trending_words_validation(query_params))
            .await
            .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_today_trending_words(method, host, cookies, query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::trend::GetTodayTrendingWordsResponse::Status200_OK(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
#[tracing::instrument(skip_all)]
async fn get_trending_words_for_day<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    ...
    A: apis::trend::Trend,
{
    ...
    let validation = tokio::task::spawn_blocking(move || {
        get_trending_words_for_day_validation(path_params, query_params)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_trending_words_for_day(method, host, cookies, path_params, query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::trend::GetTrendingWordsForDayResponse::Status200_OK(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
#[tracing::instrument(skip_all)]
async fn get_trending_words_for_month<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    ...
    A: apis::trend::Trend,
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        get_trending_words_for_month_validation(path_params, query_params)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_trending_words_for_month(method, host, cookies, path_params, query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::trend::GetTrendingWordsForMonthResponse::Status200_OK(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
#[tracing::instrument(skip_all)]
async fn get_trending_words_for_year<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    ...
    A: apis::trend::Trend,
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        get_trending_words_for_year_validation(path_params, query_params)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .get_trending_words_for_year(method, host, cookies, path_params, query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::trend::GetTrendingWordsForYearResponse::Status200_OK(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(models::WordsGetQueryParams,), ValidationErrors> {
    query_params.validate()?;
...
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || words_get_validation(query_params))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .words_get(method, host, cookies, query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::words::WordsGetResponse::Status200_SuccessfulRetrieval(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(models::WordsMeGetHeaderParams,), ValidationErrors> {
    header_params.validate()?;
    ...
    A: apis::words::Words,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || words_me_get_validation(header_params))
        .await
        .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .words_me_get(method, host, cookies, header_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::words::WordsMeGetResponse::Status200_SuccessfulRetrieval(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
fn words_post_validation(
    ...
    body: models::NewWord,
) -> std::result::Result<(models::WordsPostHeaderParams, models::NewWord), ValidationErrors> {
    header_params.validate()?;
    ...
    A: apis::words::Words,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation =
        tokio::task::spawn_blocking(move || words_post_validation(header_params, body))
            .await
            .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .words_post(method, host, cookies, header_params, body)
        .await;
...
#[tracing::instrument(skip_all)]
async fn words_word_id_delete<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    headers: HeaderMap,
    ...
    A: apis::words::Words,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        words_word_id_delete_validation(header_params, path_params)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .words_word_id_delete(method, host, cookies, header_params, path_params)
        .await;
...
#[tracing::instrument(skip_all)]
async fn words_word_id_put<I, A>(
    method: Method,
    host: Host,
    cookies: CookieJar,
    headers: HeaderMap,
    ...
    A: apis::words::Words,
{
    ...
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        words_word_id_put_validation(header_params, path_params, body)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .words_word_id_put(method, host, cookies, header_params, path_params, body)
        .await;
